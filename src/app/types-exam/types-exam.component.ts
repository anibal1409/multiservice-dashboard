...
export class TypesExamComponent implements OnInit, OnDestroy {
  ...
  clickAction(option: OptionAction) {
    switch (option.option.value) {
      case RowActionTypeExam.update:
        this.showModal(+option.data['id']);
        break;
      case RowActionTypeExam.delete:
  ...
  showConfirm(item: TypeExamVM): void {
    const dialogRef = this.matDialog.open(ConfirmModalComponent, {
      data: {
        message: {
          title: 'Eliminar tipo de examen',
          body: `¿Está seguro que desea eliminar el tipo de examen <strong>${item.name}</strong>?`,
        },
      },
      hasBackdrop: true,
      disableClose: true,
